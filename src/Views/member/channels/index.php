...
<?php if ($data['error_message']): ?>
    <div class="alert alert-danger"><?= nl2br(htmlspecialchars($data['error_message'])) ?></div>
<?php endif; ?>
<?php if ($data['success_message']): ?>
    <div class="alert alert-success"><?= nl2br(htmlspecialchars($data['success_message'])) ?></div>
<?php endif; ?>

<?php if ($data['current_channel']): ?>
    <!-- Management View -->
    <div class="dashboard-card" style="margin-top: 20px;">
        <h3>Konfigurasi Saat Ini</h3>
        <p>
            <strong>Bot Terhubung:</strong> @<?= htmlspecialchars($data['bot_details']['username'] ?? 'N/A') ?><br>
            <strong>Channel Jualan:</strong> <?= htmlspecialchars($data['current_channel']['title'] ?? 'N/A') ?> (<code><?= htmlspecialchars($data['current_channel']['channel_id']) ?></code>)<br>
            <strong>Grup Diskusi:</strong> <?= htmlspecialchars($data['current_channel']['group_title'] ?? 'N/A') ?> (<code><?= htmlspecialchars($data['current_channel']['discussion_group_id']) ?></code>)<br>
        </p>
        <hr>
...
<div class="dashboard-card" style="margin-top: 20px;">
    <h3><?= $data['current_channel'] ? 'Perbarui Konfigurasi' : 'Daftarkan Konfigurasi Baru' ?></h3>
    <form action="/member/channels" method="POST">
        ...
        <div style="margin-bottom: 15px;">
            ...
            <input type="text" id="channel_identifier" name="channel_identifier" required value="<?= htmlspecialchars($data['current_channel']['channel_id'] ?? '') ?>" placeholder="@channel_jualan_anda">
        </div>
        <div style="margin-bottom: 15px;">
            <label for="group_identifier"><strong>ID atau Username Grup Diskusi</strong></label>
            <input type="text" id="group_identifier" name="group_identifier" required value="<?= htmlspecialchars($data['current_channel']['discussion_group_id'] ?? '') ?>" placeholder="@grup_diskusi_anda">
