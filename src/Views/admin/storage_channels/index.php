...
<table>
	...
	<tbody>
		...
		<tr data-row-id="<?php echo $channel['channel_id']; ?>">
			...
			<td>
				<button class="btn btn-primary manage-bots-btn" data-channel-db-id="<?php echo $channel['id']; ?>" data-channel-id="<?php echo $channel['channel_id']; ?>" data-channel-name="<?php echo htmlspecialchars($channel['name']); ?>" data-is-default="<?php echo $channel['is_default']; ?>">
					Kelola & Edit
				</button>
			</td>
			<td>
...
<script>
	document.addEventListener('DOMContentLoaded', function() {
		...
		addBotForm.addEventListener('submit', async function(e) {
			...
			const data = await apiRequest('/api/admin/storage_channels/bots/add', {
		...
		botListDiv.addEventListener('click', async function(e) {
			...
			if (target.classList.contains('btn-verify')) {
				...
				const data = await apiRequest('/api/admin/storage_channels/bots/verify', {
					channel_id: channelId,
					bot_id: botId
				});
				showNotification(data.message, data.status === 'success' ? 'success' : 'danger');
			...
			if (target.classList.contains('btn-remove')) {
				...
				target.disabled = true;
				const botId = target.dataset.botId;
				const data = await apiRequest('/api/admin/storage_channels/bots/remove', {
					channel_id: channelId,
					bot_id: botId
				});
				showNotification(data.message, data.status === 'success' ? 'success' : 'danger');
