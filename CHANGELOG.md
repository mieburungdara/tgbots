...
### Diperbaiki (Keamanan)
- **Pengerasan Validasi Input**: Menambahkan pemeriksaan `is_string()` pada input password di `XorAdminController`. Ini mencegah `hash_equals()` mengeluarkan peringatan (warning) jika password dikirim sebagai tipe data lain (misalnya, array), yang dapat terjadi pada permintaan yang dimanipulasi. (#306)
